...
class OTRSTestClient:
    def __init__(self):
        self.base_url = os.environ.get("OTRS_BASE_URL")
        self.username = os.environ.get("OTRS_USERNAME")
        self.password = os.environ.get("OTRS_PASSWORD")
    ...
    async def __aenter__(self):
        self.session = httpx.AsyncClient(
            verify=self.verify_ssl, timeout=30, follow_redirects=True
        )
        return self
    ...
    async def session_create(self):
        ...
        auth_data = {"UserLogin": self.username, "Password": self.password}
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if self.session_id:
                    print(
                        f"✅ SessionCreate successful! SessionID: {self.session_id[:20]}..."
                    )
                    return result
                else:
    ...
    async def make_api_request_with_auth(self, operation: str, data: dict = None):
        ...
        request_data = {"UserLogin": self.username, "Password": self.password}
        if data:
    ...
    async def ticket_create(
        self, title: str, body: str, queue: str = None, customer_user: str = None
    ):
        ...
        for priority in priority_variations:
            ...
            print(
                f"🎫 Trying TicketCreate with priority: {priority}, customer: {customer_user}"
            )
            ...
            if not result.get("Error"):
                ...
                result.get("Error", {})
            ) and "CustomerUser" not in str(result.get("Error", {})):
    ...
    async def ticket_get(self, ticket_id: str):
        ...
        ticket_data = {"TicketID": ticket_id, "DynamicFields": 1, "Extended": 1}
    ...
    async def ticket_update(self, ticket_id: str, **updates):
        ...
        if "Priority" in updates:
            ...
            for priority in priority_variations:
                ...
                result = await self.make_api_request_with_auth(
...
async def test_all_otrs_operations():
    ...
    async with OTRSTestClient() as client:
        ...
        if create_result.get("TicketID"):
            ...
            search_result
            and search_result.get("TicketID")
            and len(search_result["TicketID"]) > 0
        ):
        ...
        if ticket_id:
            ...
            update_result = await client.ticket_update(
                ticket_id, State="open"  # Just update state, avoid priority issues
            )
